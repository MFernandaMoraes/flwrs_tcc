...
// Verificar se já está logado via token "lembrar de mim"
if (!isset($_SESSION['usuario_id']) && isset($_COOKIE['lembrar_token'])) {
    ...
    try {
        $sql = "SELECT u.id, u.nome_completo, u.email, lt.token 
                FROM lembrar_tokens lt 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email)) {
        ...
        try {
            ...
            $sql = "SELECT id, nome_completo, email, senha_hash, ativo 
                    FROM usuarios 
                    WHERE email = :email";
            ...
            if ($stmt->rowCount() === 0) {
                ...
                elseif (!password_verify($senha, $usuario['senha_hash'])) {
                    ...
                    $update_stmt->execute([':id' => $usuario['id']]);
                    
                    // Processar "lembrar de mim"
                    if ($lembrar) {
                        // Gerar token único
                        ...
                        $delete_sql = "DELETE FROM lembrar_tokens WHERE usuario_id = :usuario_id";
                        $delete_stmt = $conn->prepare($delete_sql);
                        $delete_stmt->execute([':usuario_id' => $usuario['id']]);
                        
                        // Inserir novo token
                        $insert_sql = "INSERT INTO lembrar_tokens (usuario_id, token, expira_em) 
                        ...
                        // Configurar cookie (30 dias)
                        setcookie('lembrar_token', $token, time() + (30 * 24 * 60 * 60), '/', '', false, true);
                    }
...
<head>
  ...
  <style>
    * {
    ...
    /* ===== HEADER (navbar com link de login/cadastro - agora sem destaque verde) ===== */
    header {
